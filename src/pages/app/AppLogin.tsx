...
export function AppLogin() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Mock login: always accept then reload (reloads will trigger useAuth fake logic)
      setTimeout(() => {
        localStorage.setItem('mockLoggedIn', id);
      ...
      <div className="flex-1 flex flex-col items-center justify-center">
        ...
        {mode === 'login' && (
          <form onSubmit={handleLogin} className="w-full max-w-sm mx-auto flex flex-col gap-4">
             ...
             {error && (
               <div className="p-3 bg-red-900/40 border border-red-500/50 text-red-300 rounded-xl text-xs font-medium w-full text-center">
                 {error}
               </div>
             )}
             <input 
                ...
                className="w-full bg-zinc-900 border border-zinc-800 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500 transition-colors"
              />
              <input 
                type="password" 
                placeholder="비밀번호" 
              ...
              <button 
                type="submit"
                disabled={loading}
                className="w-full bg-cyan-500 text-black font-black py-4 rounded-xl mt-4 hover:scale-[1.02] active:scale-[0.98] transition-all disabled:opacity-50 disabled:scale-100"
              >
                {loading ? '로그인 중...' : '로그인'}
              </button>
          </form>
